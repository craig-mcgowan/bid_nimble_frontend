...
<div class = "w-11/12 bg-neutral-100 dark:bg-slate-500 border-sky-600 border-2 rounded-md shadow-zinc-900 shadow-md rounded-t-md">
  <!-- -----------------------------Existing Trades --------------------------------- -->
  
    {#each $trades as trade (trade._id)}
    <div transition:slide class= {`grid gap-y-1 bg-white dark:bg-slate-600 rounded my-1 p-2 grid-cols-8 ${$visibleTrades.includes(trade.name)? "": "hidden"}`} class:hovRed = {hovRed===trade._id}>
